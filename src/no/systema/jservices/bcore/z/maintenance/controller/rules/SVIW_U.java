 ...
 */
public class SVIW_U {
	private static Logger logger = LoggerFactory.getLogger(SVIW_U.class.getName());
	...
	public boolean isValidInput(SviwDao dao, String user, String mode) {
		...
		if ((user != null && !"".equals(user)) && (mode != null && !"".equals(mode))) {
			if ( dao.getSviw_knnr() > 0 && StringUtils.hasValue(dao.getSviw_knso())) 	{
				...
				//Taricnr
				if ( StringUtils.hasValue(dao.getSviw_vata()) &&  !svtx10fDaoService.taricImportNrExist(dao.getSviw_vata()) ) {
					errors.append(jsonWriter.setJsonSimpleErrorResult(user,
