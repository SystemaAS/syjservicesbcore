...
public class JsonResponseOutputterController_HEDUMMY {
	private static Logger logger = LoggerFactory.getLogger(JsonResponseOutputterController_HEDUMMY.class.getName());
	...
	/**
	 *
	 * 	 File: 		SYSPEDF.HEDUMMY with MEMBER: URD (AIR EXPORT) 
	 * 	 PGM:		?		
	 * 	 Member: 	URD - Maintenance - SELECT LIST or SELECT SPECIFIC
	 *  
	 * 
	 * @return
	 * @Example SELECT *: http://gw.systema.no:8080/syjservicesbcore/syjsSYHEDUMMY_URDR.do?user=OSCAR
	 * @Example SELECT specific: http://gw.systema.no:8080/syjservicesbcore/syjsSYHEDUMMY_URDR.do?user=OSCAR&heavd=1
	 * 
	 */
	
	@RequestMapping(value="syjsSYHEDUMMY_URDR.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String syjsRList_URD( HttpSession session, HttpServletRequest request) {
		JsonResponseWriter jsonWriter = new JsonResponseWriter();
		StringBuffer sb = new StringBuffer();
		
		try{
			logger.info("Inside syjsSYHEDUMMY_URDR.do");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			String user = request.getParameter("user");
          ...
          String errMsg = "";
			...
			if(userName!=null && !"".equals(userName)){
	            ...
	            //prepare service (mandatory)
	            //-----------------------------
	            this.hedummyDaoServices.setAlias("urd_alias");
	            this.hedummyDaoServices.setMemberTable("hedummy");
	            this.hedummyDaoServices.setMember("urd");
